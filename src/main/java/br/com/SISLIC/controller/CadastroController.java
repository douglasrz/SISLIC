	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if(acao.equals("cadastrar")) {
			...
			if(forDAO.cadastrar(forn)) {
				resp.getWriter().print("<script> window.alert('Solicitação realizada com sucesso, seu cadastro será analisado. Após a análise você será notificado por e-mail, para poder usar o sistema. '); location.href='login.html';</script>");
			}else {
				resp.getWriter().print("<script> window.alert('Erro ao efetuar a solicitação de cadastro, talvez esse login já existe, tente outro'); location.href='registroForn.html';</script>");
			}
		}else {
